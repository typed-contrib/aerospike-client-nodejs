...
import status = Aerospike.status
import filter = Aerospike.filter
import GeoJSON = Aerospike.GeoJSON
import Key = Aerospike.Key
...
function executeQuery(client: Aerospike.Client, done: Function) {
  var count = 0
  ...
  var options = { filters: [filter.geoWithin('loc', region)] } as Aerospike.Client.QueryPolicy;
...
function insertRecords(client: Aerospike.Client, ndx: number, end: number, done: Function) {
  if (ndx >= end) {
...
function createIndex(client: Aerospike.Client, done: Function) {
  var options = {
    ns: argv.namespace,
    set: argv.set,
    bin: 'loc',
    index: geoIndex
  } as Aerospike.Client.CreateTypedIndexOptions;
...
function removeIndex(client: Aerospike.Client, done: Function) {
...
function removeRecords(client: Aerospike.Client, ndx: number, end: number, done: Function) {
  if (ndx >= end) {
...
function cleanup(client: Aerospike.Client, done: Function) {
