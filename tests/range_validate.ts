...
Aerospike.connect(config, function (err, client) {
  ...
  function putDone(client: Aerospike.Client, start: number, end: number) {
    ...
    return function (err: Aerospike.AerospikeError, key: Aerospike.Key) {
      inFlight--
      if (err) {
  ...
  function putStart(client: Aerospike.Client, start: number, end: number) {
  ...
  function getDone(client: Aerospike.Client, start: number, end: number) {
    var total = end - start + 1
    var done = 0
    ...
    return function (err: Aerospike.AerospikeError, record: any, metadata: Aerospike.Metadata, key: Aerospike.Key) {
      inFlight--
      done++
      if (err) {
        console.log('ERR - ', err, key)
  ...
  function getStart(client: Aerospike.Client, start: number, end: number) {
